...
#############################################################################
# Integrated population model (IPM) for Utqiagvik DUNL
...
model {
    ...
    for (t in 1:nyears) {
      j[t] ~ dpois(rho1[t])     # number eggs hatched (observed hatchlings)
      rho1[t] <- f[t] * B[t]    # number of nests monitored (B) and fecundity (f)
      }
...
jags.data <- list(zpdo = zpdo, zsnow = zsnow, fox_high = fox_high[3:20], 
                  ...
                  y = y, j = j, B = B, z = (seq(min(snow), max(snow), length.out = 100)-mean(snow))/sd(snow), nz = 100)
y
...
sum(dunl$sims.list$mlam.fox > dunl$sims.list$mlam.no)/30000
