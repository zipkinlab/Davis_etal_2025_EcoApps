...
#############################################################################
# Integrated population model (IPM) for Utqiagvik amgp
...
model {
    ...
    # Priors for mean demographic parameters
    mphia ~ dbeta(3,3)
    ...
    for (t in 2:nyears){
    log(gamma[t]) <- l.mgamma + beta.gam1  * zsnow[t] + 
                    beta.gam2 * (zsnow[t]^2) + beta.gam3 * ((f[t-1] - mean(f))/sd(f))
    ...
    # Model for initial stage-spec. population sizes (year 1)
    Nad[1] ~ dpois(y[1])
...
jags.data <- list(zpdo = zpdo, zsnow = zsnow, fox_high = fox_high[3:20], 
                  ...
                  y = y, j = j, B = B, z = seq(min(zsnow), max(zsnow), length.out = 100), 
                  zp = seq(min(zpdo), max(zpdo), length.out = 100), nz = 100)
...
sum(amgp$sims.list$mlam.fox > amgp$sims.list$mlam.no)/30000
